...
const Admin = () => {
  ...
  const fetchCountData = async() =>{
    await axios.get('http://localhost:8180/fetch-users').then(
      (response)=>{
        setUserCount(response.data.length - 1);
      }
    )
    await axios.get('http://localhost:8180/fetch-products').then(
      (response)=>{
        setProductCount(response.data.length);
      }
    )
    await axios.get('http://localhost:8180/fetch-orders').then(
      (response)=>{
  ...
  const updateBanner = async() =>{
    await axios.post('http://localhost:8180/update-banner', {banner}).then(
      (response)=>{
