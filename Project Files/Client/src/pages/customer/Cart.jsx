...
const Cart = () => {
  ...
  const fetchCart = async() =>{
    ...
    if(userId){
      await axios.get('http://localhost:8180/fetch-cart').then(
        (response)=>{
  ...
  const increaseCartQuantity = async(id) =>{
    await axios.put('http://localhost:8180/increase-cart-quantity', {id}).then(
      (response)=>{
        fetchCart();
      }
    ).catch((err)=>{
  ...
  const decreaseCartQuantity = async(id, quantity) =>{
    if(quantity > 1){
      await axios.put('http://localhost:8180/decrease-cart-quantity', {id}).then(
        (response)=>{
          fetchCart();
        }
      ).catch((err)=>{
  ...
  const removeItem = async(id) =>{
      await axios.put('http://localhost:8180/remove-item', {id}).then(
        (response)=>{
          fetchCart();
        }
      )
  ...
  const placeOrder = async() =>{
    if(cartItems.length > 0){
        await axios.post('http://localhost:8180/place-cart-order', {userId, name, mobile, email, address, pincode, paymentMethod, orderDate: new Date()}).then(
          (response)=>{
            alert('Order placed!!');
            setName('');
            setMobile('');
